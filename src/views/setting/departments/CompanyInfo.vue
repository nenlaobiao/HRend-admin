<template>
  <el-form label-width="80px" style="width: 400px">
    <el-form-item label="公司名称">
      <el-input disabled :value="info.name" />
    </el-form-item>
    <el-form-item label="公司地址">
      <el-input disabled :value="info.companyAddress" />
    </el-form-item>
    <el-form-item label="公司电话">
      <el-input disabled :value="info.companyPhone" />
    </el-form-item>
    <el-form-item label="邮编">
      <el-input disabled :value="info.mailbox" />
    </el-form-item>
    <el-form-item label="备注">
      <el-input disabled :value="info.remarks" />
    </el-form-item>
  </el-form>
</template>

<script>
import { getCompanyInfo } from '@/api/setting'
import { mapGetters } from 'vuex'
export default {
  filters: {},
  components: {},
  data () {
    return {
      info: {}
    }
  },
  computed: {
    ...mapGetters(['companyId'])
  },
  watch: {},
  async created () {
    console.log(this.$store.state.user)
    const res = await getCompanyInfo(this.companyId)
    console.log(res)
    this.info = res
  },
  methods: {}
}
</script>

<style scoped lang='scss'>
</style>
